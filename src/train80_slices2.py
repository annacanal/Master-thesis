...
def main():
    ...
    check = ModelCheckpoint("/tmp/cae80_slices2.hdf5", monitor= 'val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)

    tensorboard = TensorBoard(log_dir="logs80slice2/{}".format(time()), histogram_freq=1, batch_size=30, write_graph=True, write_grads=True,
    ...
    for i,element in enumerate(images_train):
        train[i] = element[5:85,55,5:85]
    #test images
    for i,element in enumerate(images_test):
        test[i] = element[5:85,55,5:85]
    ...
    for n_batch in batch:
        ...
        for lr in learning_rate:
            ...
            np.save("cae80slices2_512nodes_"+str(n_batch)+"_lr"+str(lr), compressed512)
            np.save("cae80slices2_128nodes_" + str(n_batch) + "_lr" + str(lr), compressed128)
