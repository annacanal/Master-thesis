...
def encoder(model, input_img_size ):
    model.add(Conv2D(8, (3, 3), padding='same', input_shape= input_img_size)) # apply 8 filters sized of (3x3x3)
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))  # strides=(2,2,2)
    # 2nd conv
...
def decoder(model ):
    ...
    model.add(Reshape((10, 10, 8)))
    # 4th conv
    model.add(Conv2D(8, (3, 3), padding='same'))#, input_shape=input_img_size))
    model.add(Activation('relu'))
